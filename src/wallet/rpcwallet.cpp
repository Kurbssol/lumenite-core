...
static RPCHelpMan loadwallet()
{
    return RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting lumenited will be"
        ...
        [want_psbt](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !want_psbt) {
        if (!pwallet->chain().rpcEnableDeprecated("bumpfee")) {
            throw JSONRPCError(RPC_METHOD_DEPRECATED, "Using bumpfee with wallets that have private keys disabled is deprecated. Use psbtbumpfee instead or restart lumenited with -deprecatedrpc=bumpfee. This functionality will be removed in 0.22");
        }
        want_psbt = true;
    }
